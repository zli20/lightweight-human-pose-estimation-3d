...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--model',
                        ...
                        type=str, default=r"C:\Users\26522\Downloads\human-pose-estimation-3d.pth")
    ...
    parser.add_argument('--use-tensorrt', help='Optional. Run network with TensorRT as inference engine.',
                        action='store_true')
    parser.add_argument('--images', help='Optional. Path to input image(s).', nargs='+', default='["C:\\Users\\26522\\Desktop\\R.jpg"]')
    ...
    video_path = r"E:\svn_suanfa\2D_Demo\taiji.mp4"
    ...
    while True:
        ...
        det_scale = base_height / frame.shape[0]
        scaled_img = cv2.resize(frame, dsize=None, fx=input_scale, fy=input_scale)
        scaled_img = scaled_img[:, 0:scaled_img.shape[1] - (scaled_img.shape[1] % stride)]  # better to pad, but cut out for de

        # _imgSize = (448, 256)  # 替换为目标图像的大小
        #
        # det_im = np.zeros((_imgSize[1], _imgSize[0], 3), dtype=np.float32)
        # # 计算图像比例
        # input_scale = frame.shape[0] / frame.shape[1]
        # if input_scale > 1:
        #     new_height = _imgSize[1]
        #     new_width = int(new_height / input_scale)
        # else:
        #     new_width = _imgSize[0]
        #     new_height = int(new_width * input_scale)
        # # 计算尺度因子
        # det_scale = new_height / frame.shape[0]
        # # 调整大小
        # resized_img = cv2.resize(frame, (new_width, new_height))
        # # cv2.imshow('ion1', resized_img)
        # # 将调整后的图像复制到空白图像中
        # det_im[0:new_height, 0:new_width, :] = resized_img
        # # 归一化到 [0, 255] 范围并转换为 uint8 类型
        # scaled_img = cv2.normalize(det_im, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
        # cv2.imshow('ion', scaled_img)
        # cv2.waitKey(0)
        ...
        poses_3d, poses_2d = parse_poses(inference_result, det_scale, stride, fx, is_video)
        edges = []

        # with open(file_path, 'a') as file:
        #     point_s = poses_3d[0].reshape(19, 4)
        #     for point in point_s:
        #         line = ' '.join(map(str, point[0:3]))
        #         file.write(line + '\n')
        # file.close()
