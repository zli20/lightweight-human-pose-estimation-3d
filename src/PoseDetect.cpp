...
void PoseDetect::postProcess()
{
	...
	for (size_t person_id = 0; person_id < num_persons; person_id++)
	{
		...
		std::vector<float> pose2d((NUM_POINTS + 1) * 3 + 1, -1);
		for (size_t kpt_id = 0; kpt_id < num_keypoints; kpt_id++)
		{
			int map_id = OPENPOSE_ID_TO_PANOPTIC[kpt_id];
		...
		if (pose2d[2] > POINTS_THRESHOLD) {
			...
			for (auto limb : LIMBS) {
				for (auto kpt_id_from : limb) {
					if (pose2d[kpt_id_from * 3 + 2] > POINTS_THRESHOLD) {
...
std::vector<cv::Mat> PoseDetect::wrap_feature_maps(float* heatmapPtr, int num_channels, int height, int width) {
...
float PoseDetect::computeTrace(const std::vector<std::vector<float>>& matrix) {
	float trace = 0.0;
...
void PoseDetect::rotate_poses(std::vector<float>& poses_3d, cv::Mat R, cv::Mat t) {
	cv::Mat R_inv;
...
#if 0
void PoseDetect::savePose3dToTxt(std::vector<std::vector<float>>& poses, const std::string& filename) {
	// 打开文件，续写
...
void PoseDetect::saveDataToTxt(const float* dataPtr, int dataSize, const std::string& filename) {
